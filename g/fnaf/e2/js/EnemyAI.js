...
class EnemyAI {
    constructor(game) {
        ...
        this.diddy = {
            ...
            hasSpawned: false,
            moving: false,
            path: ['cam5', 'cam6', 'cam3', 'cam1', 'office'],
            pathIndex: 0,
            movementTimer: null,
            oilUsed: 0,
            currentImage: null,
    ...
    // Diddy 出场
    spawnDiddy() {
        if (this.diddy.hasSpawned) return;
        
        this.diddy.hasSpawned = true;
        this.diddy.currentLocation = 'cam5';
        this.diddy.pathIndex = 0;
        this.diddy.moving = false;
        this.diddy.oilUsed = 0;
        this.pickDiddyImage();
        console.log('Diddy has spawned at cam5!');
    ...
    stop() {
        ...
        if (this.diddy.movementTimer) {
            clearTimeout(this.diddy.movementTimer);
            this.diddy.movementTimer = null;
        }
    }
    ...
    giveDiddyOil() {
        ...
        this.diddy.oilUsed += 1;
        console.log(`Diddy oil used: ${this.diddy.oilUsed}`);
        
        if (this.diddy.oilUsed >= 2) {
            this.triggerCameraFailure();
        }
        
        if (!this.diddy.moving) {
            this.startDiddyMovement();
        }
    }

    startDiddyMovement() {
        this.diddy.moving = true;
        this.scheduleDiddyMove();
    }

    scheduleDiddyMove() {
        if (!this.diddy.moving) return;
        
        const interval = 8000;
        this.diddy.movementTimer = setTimeout(() => {
            this.advanceDiddy();
        }, interval);
    }
    ...
    advanceDiddy() {
        ...
        if (this.diddy.pathIndex < this.diddy.path.length - 1) {
            this.diddy.pathIndex += 1;
            this.diddy.currentLocation = this.diddy.path[this.diddy.pathIndex];
            
            if (this.diddy.currentLocation === 'cam5') {
                this.pickDiddyImage();
            } else {
                this.diddy.currentImage = null;
            }
            
            if (this.diddy.currentLocation === 'office') {
                this.triggerJumpscare('diddy');
                return;
            }
    ...
    // 触发跳杀
    triggerJumpscare(enemy = 'epstein', options = {}) {
        console.log(`JUMPSCARE by ${enemy}!`);

        const ignoreMask = options.ignoreMask === true;
        
        if (!ignoreMask && this.game.state.maskOn && (enemy === 'epstein' || enemy === 'trump')) {
            console.log('Mask on - jumpscare blocked.');
            this.repelMaskedEnemy(enemy);
            return;
        }
        ...
        if (enemy === 'diddy') {
            ...
            jumpscareContainer.appendChild(jumpscareGif);
            document.body.appendChild(jumpscareContainer);
            
            setTimeout(() => {
                if (document.body.contains(jumpscareContainer)) {
    ...
    repelMaskedEnemy(enemy) {
        if (enemy === 'epstein') {
            this.epstein.currentLocation = 'cam1';
            this.epstein.hasSpawned = true;
            this.updateCameraDisplay();
            return;
        }
        ...
        if (enemy === 'trump') {
            ...
            this.trump.isCrawling = false;
            this.trump.crawlingFrom = null;
            this.trump.currentLocation = 'cam1';
            this.trump.hasSpawned = true;
            this.game.assets.stopSound('ventCrawling');
            this.updateCameraDisplay();
        }
    ...
    // 获取 Diddy 当前位置
    getDiddyCurrentLocation() {
        return this.diddy.currentLocation;
    }

    pickDiddyImage() {
        const idx = Math.floor(Math.random() * this.diddyImages.length);
        this.diddy.currentImage = this.diddyImages[idx];
    }

    getDiddyImage() {
        if (!this.diddy.currentImage) {
            this.pickDiddyImage();
        }
        return this.diddy.currentImage;
    }

    getDiddyBrightness() {
        return this.characterBrightness['cam5'] || 100;
    }

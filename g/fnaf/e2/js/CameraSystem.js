...
class CameraSystem {
    constructor(game) {
        ...
        this.giveOilBtn = document.getElementById('give-oil-btn');
    ...
    bindEvents() {
        ...
        if (this.giveOilBtn) {
            this.giveOilBtn.addEventListener('click', () => this.giveDiddyOil());
        }
    }
    ...
    updateCharacterDisplay() {
        ...
        const diddyLocation = this.game.enemyAI.getDiddyCurrentLocation ? this.game.enemyAI.getDiddyCurrentLocation() : null;
        
        console.log(`updateCharacterDisplay - Current Cam: ${currentCam}, EP: ${epLocation}, Trump: ${trumpLocation}, Hawking: ${hawkingActive}, Diddy: ${diddyLocation}, Night: ${this.game.state.currentNight}`);
        ...
        // 显示 Diddy（仅 cam5 可见）
        if (this.game.enemyAI.diddy && this.game.enemyAI.diddy.hasSpawned && currentCam === 'cam5' && diddyLocation === 'cam5') {
            const diddyImg = document.createElement('img');
            diddyImg.src = this.game.enemyAI.getDiddyImage();
            diddyImg.style.position = 'absolute';
            diddyImg.className = 'visible diddy-character';
            diddyImg.style.zIndex = '2';
            diddyImg.style.left = '50%';
            diddyImg.style.bottom = '0%';
            diddyImg.style.width = '32%';
            diddyImg.style.transform = 'translateX(-50%)';
            
            const brightness = this.game.enemyAI.getDiddyBrightness ? this.game.enemyAI.getDiddyBrightness() : 100;
            diddyImg.style.filter = `brightness(${brightness}%)`;
            
            characterOverlay.appendChild(diddyImg);
            console.log('✓ Displaying Diddy at cam5');
        }
    ...
    updateGiveOilVisibility() {
        if (!this.giveOilBtn) return;
        
        const show = this.game.state.cameraOpen && this.game.state.currentCam === 'cam5';
        this.giveOilBtn.style.display = show ? 'block' : 'none';
    }

    giveDiddyOil() {
        if (this.game.enemyAI && this.game.enemyAI.giveDiddyOil) {
            this.game.enemyAI.giveDiddyOil();
        }
    }

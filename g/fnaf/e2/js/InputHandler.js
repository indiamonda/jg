...
class InputHandler {
    ...
    bindEvents() {
        ...
        gameScreen.addEventListener('mouseleave', (e) => this.handleMouseLeave(e));
    ...
    handleMouseMove(e) {
        ...
        this.game.updateFlashlightPosition(e);
        
        const isLeftDown = (e.buttons & 1) === 1;
        if (isLeftDown) {
            const rect = this.game.gameScreen.getBoundingClientRect();
            const x = e.clientX - rect.left;
            const y = e.clientY - rect.top;
            if (!this.game.tryStartCrank(x, y)) {
                this.game.stopCrank();
                this.game.setFlashlightOn(true);
            } else {
                this.game.setFlashlightOn(false);
            }
        } else {
            this.game.stopCrank();
        }
        
        if (this.game.state.cameraOpen) return;

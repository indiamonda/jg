...
class Game {
    constructor() {
        ...
        this.flashlightPos = { x: window.innerWidth * 0.5, y: window.innerHeight * 0.5 };
        this.hallwayInterval = null;
        this.hallwayEnemyEndTime = 0;
        this.hallwayEnemyCooldownEnd = 0;
        this.rewinderMax = 30;
        this.isCranking = false;
        this.crankCoord = { x: 156 / 1799, y: 324 / 766 };
        this.crankCenterPx = { x: 0, y: 0 };
        this.crankRadius = 45;
    ...
    initElements() {
        ...
        this.maskOverlayEl = document.getElementById('mask-overlay');
        this.hallwayEnemyEl = document.getElementById('hallway-enemy');
        this.crankEl = document.getElementById('crank-handle');
    ...
    async initGame() {
        ...
        this.state.reset();
        this.resetFlashlightAndMask();
        this.clearHallwayEnemy();
        if (this.gameScreen) {
            const rect = this.gameScreen.getBoundingClientRect();
            this.flashlightPos = { x: rect.width * 0.5, y: rect.height * 0.5 };
            if (this.flashlightEl) {
                this.flashlightEl.style.left = `${this.flashlightPos.x}px`;
                this.flashlightEl.style.top = `${this.flashlightPos.y}px`;
            }
        }
        ...
        if (type === 'night2') {
            ...
            tutorialContent.innerHTML = `
                ...
                <p>
                    ...
                    ALSO, DIDDY IS IN CAM 5 HE WILL NOT ATTACK UNLESS YOU DON'T GIVE HIM BABY OIL FOR TOO LONG.
                </p>
                <button id="tutorial-got-it">GOT IT</button>
            `;
            // 重新绑定按钮事件
    ...
    startGameLoop() {
        ...
        this.hallwayInterval = setInterval(() => {
            this.updateHallwayPresence();
        }, 500);
    }
    ...
    updateRewinder() {
        ...
        if (this.isCranking && !this.state.maskOn && !this.state.cameraOpen) {
            this.state.rewinderTime = Math.min(this.rewinderMax, this.state.rewinderTime + 2);
        } else {
            this.state.rewinderTime = Math.max(0, this.state.rewinderTime - 1);
        }
        
        if (this.state.rewinderTime <= 0 && !this.state.rewinderTriggered) {
            this.state.rewinderTriggered = true;
            this.triggerRewinderBell();
        }
    }

    triggerRewinderBell() {
        this.assets.playSound('ping', false, 1.0);
        
        const candidates = [];
        if (this.enemyAI.epstein.hasSpawned) candidates.push('epstein');
        if (this.enemyAI.trump.hasSpawned) candidates.push('trump');
        const enemy = candidates.length > 0 ? candidates[Math.floor(Math.random() * candidates.length)] : 'epstein';
        
        this.enemyAI.triggerJumpscare(enemy, { ignoreMask: true });
    }

    updateCrankPosition(viewPosition = this.viewPosition) {
        if (!this.crankEl || !this.gameScreen) return;
        
        const screenWidth = this.gameScreen.clientWidth || window.innerWidth;
        const screenHeight = this.gameScreen.clientHeight || window.innerHeight;
        const imageWidth = screenWidth * 1.5;
        const leftOffsetPx = (-viewPosition * 0.5) * screenWidth;
        
        const xPxInImage = this.crankCoord.x * imageWidth;
        const yPxInImage = this.crankCoord.y * screenHeight;
        
        const screenX = leftOffsetPx + xPxInImage;
        const screenY = yPxInImage;
        
        this.crankCenterPx = { x: screenX, y: screenY };
        
        this.crankEl.style.left = `${screenX}px`;
        this.crankEl.style.top = `${screenY}px`;
        this.crankEl.style.opacity = this.state.cameraOpen ? '0' : '0.95';
    }

    isCrankableAt(screenX, screenY) {
        if (!this.state.isGameRunning || this.state.cameraOpen || this.state.maskOn) return false;
        
        const dx = screenX - this.crankCenterPx.x;
        const dy = screenY - this.crankCenterPx.y;
        const distance = Math.hypot(dx, dy);
        return distance <= this.crankRadius;
    }

    tryStartCrank(screenX, screenY) {
        if (this.isCrankableAt(screenX, screenY)) {
            if (!this.isCranking) {
                this.assets.playSound('crank1', false, 0.8);
            }
            this.isCranking = true;
            if (this.crankEl) {
                this.crankEl.classList.add('cranking');
            }
            return true;
        }
        return false;
    ...
    stopCrank() {
        this.isCranking = false;
        if (this.crankEl) {
            this.crankEl.classList.remove('cranking');
        }
    }

    setFlashlightOn(isOn, force = false) {
        if (!force) {
            if (!this.state.isGameRunning || this.state.cameraOpen) {
                return;
            }
        }
        ...
        if (this.flashlightEl) {
            this.flashlightEl.classList.toggle('active', isOn);
        }
        this.updateHallwayVisibility();
    }

    setMaskOn(isOn, force = false) {
        if (!force) {
            if (!this.state.isGameRunning || this.state.cameraOpen) {
                return;
            }
            if (isOn && this.isCranking) {
                return;
            }
        }
        
        this.state.maskOn = isOn;
        if (this.maskOverlayEl) {
            this.maskOverlayEl.classList.toggle('active', isOn);
        }
    }

    toggleMask() {
        if (!this.state.isGameRunning || this.state.cameraOpen) return;
        this.setMaskOn(!this.state.maskOn);
    }

    updateFlashlightPosition(e) {
        if (!this.state.isGameRunning || !this.gameScreen) return;
        
        const rect = this.gameScreen.getBoundingClientRect();
        const x = Math.max(0, Math.min(rect.width, e.clientX - rect.left));
        const y = Math.max(0, Math.min(rect.height, e.clientY - rect.top));
        
        this.flashlightPos = { x, y };
        
        if (this.flashlightEl) {
            this.flashlightEl.style.left = `${x}px`;
            this.flashlightEl.style.top = `${y}px`;
        }
    ...
    setHallwayEnemy(enemy) {
        this.state.hallwayEnemy = enemy;
        if (!this.hallwayEnemyEl) return;
        
        const basePath = this.assets.getBasePath();
        const src = enemy === 'trump' 
            ? `${basePath}assets/images/trump3.png`
            : `${basePath}assets/images/enemyep1.png`;
        
        this.hallwayEnemyEl.src = src;
    }

    clearHallwayEnemy() {
        this.state.hallwayEnemy = null;
        this.hallwayEnemyEndTime = 0;
        if (this.hallwayEnemyEl) {
            this.hallwayEnemyEl.classList.remove('visible');
            this.hallwayEnemyEl.src = '';
        }
    }

    updateHallwayPresence() {
        if (!this.state.isGameRunning) return;
        ...
        if (this.state.hallwayEnemy && now > this.hallwayEnemyEndTime) {
            this.clearHallwayEnemy();
            this.hallwayEnemyCooldownEnd = now + this.getRandomRange(3000, 7000);
        }
        
        if (!this.state.hallwayEnemy && now > this.hallwayEnemyCooldownEnd) {
            const roll = Math.random();
            if (roll < 0.08) {
                const candidates = [];
                if (this.enemyAI.epstein.hasSpawned && this.enemyAI.epstein.currentLocation !== 'office') {
                    candidates.push('epstein');
                }
                if (this.enemyAI.trump.hasSpawned && this.enemyAI.trump.currentLocation !== 'office' && !this.enemyAI.trump.isCrawling) {
                    candidates.push('trump');
                }
                
                if (candidates.length > 0) {
                    const enemy = candidates[Math.floor(Math.random() * candidates.length)];
                    this.setHallwayEnemy(enemy);
                    this.hallwayEnemyEndTime = now + this.getRandomRange(2000, 4500);
                }
            }
        }
    ...
    updateHallwayVisibility() {
        if (!this.hallwayEnemyEl) return;
        
        if (!this.state.hallwayEnemy || !this.state.flashlightOn || this.state.cameraOpen) {
            this.hallwayEnemyEl.classList.remove('visible');
            return;
        }
        
        const screenRect = this.gameScreen.getBoundingClientRect();
        const enemyRect = this.hallwayEnemyEl.getBoundingClientRect();
        const enemyCenter = {
            x: enemyRect.left + enemyRect.width * 0.5 - screenRect.left,
            y: enemyRect.top + enemyRect.height * 0.5 - screenRect.top
        };
        
        const lightRect = this.flashlightEl ? this.flashlightEl.getBoundingClientRect() : null;
        const radius = lightRect ? Math.min(lightRect.width, lightRect.height) * 0.22 : 0;
        
        const dx = enemyCenter.x - this.flashlightPos.x;
        const dy = enemyCenter.y - this.flashlightPos.y;
        const distance = Math.hypot(dx, dy);
        
        if (distance <= radius) {
            this.hallwayEnemyEl.classList.add('visible');
        } else {
            this.hallwayEnemyEl.classList.remove('visible');
        }
    }

    resetFlashlightAndMask() {
        this.setFlashlightOn(false, true);
        this.setMaskOn(false, true);
        this.stopCrank();
        this.updateHallwayVisibility();
    }
    ...
    stopGame() {
        ...
        clearInterval(this.hallwayInterval);
        this.enemyAI.stop();
    }
